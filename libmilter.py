...
def parse_packet(p):
    ...
    while p:
        if len(p) < 4:
            raise InvalidPacket(
                p, ret, 'The packet is too small to '
...
def checkData(data, macro):
    if not data[:1] == macro:
        raise UnknownError(
            'Command does not start with correct '
...
def noCallback(f):
    ...
    fname = f.__name__
    if fname not in optCBs:
        raise RequiredCallbackError(
            'function %s is NOT an optional callback' % fname)
...
class MilterProtocol(object):
    ...
    def dataReceived(self, buf):
        ...
        if self._partialHeader:
            ...
            debug(
                'Working a partial header: %r ; cmds: %r' % (pheader, cmds),
                4, self.id)
            buf = pheader + buf
            self._partialHeader = None
        if self._partial:
            ...
            cmds.extend(pcmds)
            debug(
                'Got a chunk of a partial: len: %d ; ' % buflen +
        ...
        if buf:
            ...
            try:
                ...
                debug(
                    'Found a partial header: %r; cmdlen: %d ; buf: %r' %
                ...
                cmds.extend(curcmds)
        debug(
            'parsed packet, %d cmds, %d remaining: cmds: %r ; qid: %s' %
        ...
        for i, cmd in enumerate(cmds):
            ...
            if toSend and not mtype:
                ...
                if mtype in protoMap:
                    ...
                    if self.protos & nc:
                        debug(
                            'No callback set for %r' % self._MACMAP[mtype],
                            4, self.id)
                        ...
                        # the response
                        debug(
                            'No response set for %r' % self._MACMAP[mtype],
                            4, self.id)
                ...
                debug(
                    'Calling %r for qid: %s' % (
                        self._MACMAP[mtype], self._qid), 4, self.id)
    ...
    def connect(self, hostname, family, ip, port, cmdDict):
        ...
        """
        self.log(
            'Connect from %s:%d (%s) with family: %s' % (
                ip, port, hostname, family))
        return CONTINUE
...
class AsyncFactory(object):
    # __init__() {{{
    def __init__(
            self, sockstr, protocol, opts=0, listenq=50, sockChmod=0o666):
        self.sock = None
        self.opts = opts
